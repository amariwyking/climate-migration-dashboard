...
def main():
    ...
    us_county_data['COUNTY_FIPS'] = us_county_data['STATE_FIPS'] + us_county_data['COUNTY_FIPS']
    ...
    climate_regions = {
        ...
        'South': [
            ...
            'Kentucky', 'North Carolina', 'South Carolina', 'Tennessee', 'Alabama',
            'Georgia', 'Florida', 'Arkansas', 'Mississippi', 'Louisiana', 'Oklahoma', 'Texas',
        ],
        'Midwest': [
            'Montana', 'Wyoming', 'North Dakota', 'South Dakota', 'Nebraska', 'Kansas',
            'Minnesota', 'Iowa', 'Missouri', 'Wisconsin', 'Illinois', 'Michigan',
            'Indiana', 'Ohio',
        ],
        'West': [
            'Washington', 'Oregon', 'Idaho', 'Nevada', 'Utah', 'Colorado', 'Arizona', 'New Mexico',
        ],
        'California': [
            'California'
        ]
    ...
    state_climate_regions = {state.lower(
    ): region for region, states in climate_regions.items() for state in states}
    us_county_data['CLIMATE_REGION'] = us_county_data['STATE_NAME'].str.lower().map(
        state_climate_regions)

    # Aggregate population by climate region
    climate_region_populations = us_county_data[[
        'POPULATION_2010', 'CLIMATE_REGION']].groupby(by='CLIMATE_REGION').aggregate('sum')

    # Calculate regional shares of the US population
    climate_region_shares = pd.DataFrame(climate_region_populations['POPULATION_2010'].divide(
        climate_region_populations['POPULATION_2010'].sum()).multiply(100).round(2))
    ...
    climate_region_populations['POPULATION_2065_S3'] = qf_2065_regional_population_shares['Scenario_3'].divide(100).multiply(
        census_national_population_2065).astype(int)

    # Calculate climate migration effects for different scenarios
    climate_effect_on_pop_shares = pd.DataFrame(
        qf_2065_regional_population_shares['Scenario_5'].div(
            qf_2065_regional_population_shares['Scenario_3']).sub(1),
        columns=['scenario_5_100%'],
    )

    # Define climate migration intensities
    climate_effect_on_pop_shares['scenario_5_50%'] = climate_effect_on_pop_shares['scenario_5_100%'].multiply(
        0.50)
    climate_effect_on_pop_shares['scenario_5_200%'] = climate_effect_on_pop_shares['scenario_5_100%'].multiply(
        2.0)
    ...
    alternate_scenario_5_pop_shares = climate_effect_on_pop_shares.apply(
        generate_scenario_5_variations, axis=0).div(100)
    ...
    climate_region_populations = climate_region_populations.merge(
        alternate_scenario_5_pop_shares.mul(
            census_national_population_2065).astype(int),
        left_index=True,
        right_index=True
    ).rename(
    ...
    def calculate_proportion_of_region(row):
        ...
        region_population = climate_region_populations.loc[climate_region]['POPULATION_2010']
    ...
    def calculate_county_scenario_populations(row):
        ...
        migration_scenarios = climate_region_populations[[
            'POPULATION_2065_S3', 'POPULATION_2065_S5b', 'POPULATION_2065_S5a', 'POPULATION_2065_S5c']]
        return migration_scenarios.loc[climate_region].mul(proportion_of_region).astype(int)
