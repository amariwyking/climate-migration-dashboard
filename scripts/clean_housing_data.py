...
def load_and_process_housing_data() -> pd.DataFrame:
    ...
    for file in PATHS["raw_housing"].iterdir():
        ...
        column_map = next(
            (mapping for (start, end), mapping in COLUMN_MAPPINGS.items() 
             if start <= year <= end),
            None
        )
        if not column_map:
...
def process_housing_dataframe(df: pd.DataFrame, column_map: Dict[str, str], year: int) -> pd.DataFrame:
...
def process_population_dataframe(df: pd.DataFrame, year: int) -> pd.DataFrame:
    ...
    df["Year"] = year
    return df.rename(columns={POPULATION_COLUMN: "POPULATION"})[["COUNTY_FIPS", "Year", "POPULATION"]]
